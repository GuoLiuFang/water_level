...
class PredictAfterNms(object):
    ...
    def nms(self, predict, activation_pixels, threshold=config.side_vertex_pixel_threshold):
        ...
        for group, g_th in zip(D, range(len(D))):
            ...
            for row in group:
                ...
                for ij in region_list[row]:
                    ...
                    if score >= threshold:
                        ...
                        # 说明是头像素
                        if ith_score < config.trunc_threshold:
                            headx_list.append(ij[1])
                        # 说明是尾像素
                        if ith_score >= 1 - config.trunc_threshold:
            ...
            for row in group:
                ...
                for ij in region_list[row]:
                    ...
                    if score >= threshold:
                        ith_score = predict[ij[0], ij[1], 2:3]
                        # 说明是头像素
                        if ith_score < config.trunc_threshold and ij[1] <= max_head_x:
                            ith = int(np.around(ith_score))
                            total_score[ith * 2:(ith + 1) * 2] += score
                            px = (ij[1] + 0.5) * config.pixel_size
                            py = (ij[0] + 0.5) * config.pixel_size
                            p_v = [px, py] + np.reshape(predict[ij[0], ij[1], 3:7], (2, 2))
                            quad_list[g_th, ith * 2:(ith + 1) * 2] += score * p_v
                        # 说明是尾像素
                        if ith_score >= 1 - config.trunc_threshold and ij[1] >= min_tail_x:
                            ith = int(np.around(ith_score))
                            total_score[ith * 2:(ith + 1) * 2] += score
                            px = (ij[1] + 0.5) * config.pixel_size
                            py = (ij[0] + 0.5) * config.pixel_size
            ...
            quad_list[g_th] /= (total_score + config.epsilon)
