...
class East:
    ...
    def __init__(self):
        self.input_img = Input(name='input_img',
                               shape=(None, None, config.num_channels),
                               dtype='float32')
        vgg16 = VGG16(input_tensor=self.input_img,
                      weights='imagenet',
                      include_top=False)
        if config.locked_layers:
        ...
        self.f = [vgg16.get_layer('block%d_pool' % i).output
                  for i in config.feature_layers_range]
        self.f.insert(0, None)
        self.diff = config.feature_layers_range[0] - config.feature_layers_num

    def g(self, i):
        # i+diff in cfg.feature_layers_range
        assert i + self.diff in config.feature_layers_range, \
            ('i=%d+diff=%d not in ' % (i, self.diff)) + \
            str(config.feature_layers_range)
        if i == config.feature_layers_num:
    ...
    def east_network(self):
        before_output = self.g(config.feature_layers_num)
